...
json
 ...
 1 file changed, 2960 insertions(+), 440 deletions(-)

diff --git a/drivers/hid/hid-debug.c b/drivers/hid/hid-debug.c
index 7dd83ec74..add353a17 100644
=2D-- a/drivers/hid/hid-debug.c
+++ b/drivers/hid/hid-debug.c
@@ -37,437 +37,2808 @@ struct hid_usage_entry {
 };

 static const struct hid_usage_entry hid_usage_table[] =3D {
-  {  0,      0, "Undefined" },
-  {  1,      0, "GenericDesktop" },
-    {0, 0x01, "Pointer"},
-    {0, 0x02, "Mouse"},
...
+	{ 0, 0, NULL }
 ...
 /* Either output directly into simple seq_file, or (if f =3D=3D NULL)
@@ -510,8 +2881,12 @@ static char *resolv_usage_page(unsigned page, struct=
 seq_file *f) {
...
+	const struct hid_usage_entry *m;
 	char *buf =3D NULL;
 	int len =3D 0;
+	const char *modifier =3D NULL;
+	unsigned int usage_modifier =3D usage & 0xF000;
+	unsigned int usage_actual =3D usage & 0xFFFF;

 	buf =3D resolv_usage_page(usage >> 16, f);
 	if (IS_ERR(buf)) {
@@ -529,16 +2904,33 @@ char *hid_resolv_usage(unsigned usage, struct seq_f=
ile *f) {
 	}
 	for (p =3D hid_usage_table; p->description; p++)
 		if (p->page =3D=3D (usage >> 16)) {
+			if (p->page =3D=3D 0x20 && usage_modifier) {
+				for (m =3D p; m->description; m++) {
+					if (p->page =3D=3D m->page && m->usage
+							=3D=3D usage_modifier) {
+						modifier =3D m->description;
+						break;
+					}
+				}
+				if (modifier)
+					usage_actual =3D usage_actual & 0x0FFF;
+			}
+
+			if (!modifier)
+				modifier =3D "";
+
 			for(++p; p->description && p->usage !=3D 0; p++)
-				if (p->usage =3D=3D (usage & 0xffff)) {
+				if (p->usage =3D=3D usage_actual) {
 					if (!f)
 						snprintf(buf + len,
...
+							modifier);
 			...
 			break;
@@ -753,12 +3145,12 @@ static const char *events[EV_MAX + 1] =3D {
 	[EV_MSC] =3D "Misc",			[EV_LED] =3D "LED",
 	[EV_SND] =3D "Sound",			[EV_REP] =3D "Repeat",
 	[EV_FF] =3D "ForceFeedback",		[EV_PWR] =3D "Power",
-	[EV_FF_STATUS] =3D "ForceFeedbackStatus",
+	[EV_FF_STATUS] =3D "ForceFeedbackStatus",	[EV_SW]  =3D "Software",
 };

-static const char *syncs[3] =3D {
+static const char *syncs[SYN_CNT] =3D {
 	[SYN_REPORT] =3D "Report",		[SYN_CONFIG] =3D "Config",
-	[SYN_MT_REPORT] =3D "MT Report",
+	[SYN_MT_REPORT] =3D "MT Report",		[SYN_DROPPED] =3D "Dropped",
 };

 static const char *keys[KEY_MAX + 1] =3D {
@@ -995,6 +3387,103 @@ static const char *keys[KEY_MAX + 1] =3D {
...
riggerHappy40",
+	[BTN_DIGI] =3D "Digi",			[BTN_STYLUS3] =3D "Stylus3",
+	[BTN_TOOL_QUINTTAP] =3D "ToolQuintTap",	[BTN_WHEEL] =3D "Wheel",
+	[KEY_10CHANNELSDOWN] =3D "10ChannelsDown",
+	[KEY_10CHANNELSUP] =3D "10ChannelsUp",
+	[KEY_3D_MODE] =3D "3DMode",		[KEY_ADDRESSBOOK] =3D "Addressbook",
+	[KEY_ALS_TOGGLE] =3D "ALSToggle",		[KEY_ASPECT_RATIO] =3D "AspectRatio",
+	[KEY_ATTENDANT_OFF] =3D "AttendantOff",	[KEY_ATTENDANT_ON] =3D "Attendan=
tOn",
+	[KEY_ATTENDANT_TOGGLE] =3D "AttendantToggle",
+	[KEY_AUDIO_DESC] =3D "AudioDesc",
+	[KEY_AUTOPILOT_ENGAGE_TOGGLE] =3D "AutoPiloteEngage",
+	[KEY_BATTERY] =3D "Battery",		[KEY_BLUETOOTH] =3D "BlueTooth",
+	[KEY_BRIGHTNESS_CYCLE] =3D "BrightnessCycle",
+	[KEY_BRIGHTNESS_MENU] =3D "BrightnessMenu",
+	[KEY_BRL_DOT1] =3D "BrlDot1",		[KEY_BRL_DOT10] =3D "BrlDot10",
+	[KEY_BRL_DOT2] =3D "BrlDot2",		[KEY_BRL_DOT3] =3D "BrlDot3",
+	[KEY_BRL_DOT4] =3D "BrlDot4",		[KEY_BRL_DOT5] =3D "BrlDot5",
+	[KEY_BRL_DOT6] =3D "BrlDot6",		[KEY_BRL_DOT7] =3D "BrlDot7",
+	[KEY_BRL_DOT8] =3D "BrlDot8",		[KEY_BRL_DOT9] =3D "BrlDot9",
+	[KEY_CAMERA_DOWN] =3D "CameraDown",	[KEY_CAMERA_FOCUS] =3D "CameraFocus"=
,
+	[KEY_CAMERA_LEFT] =3D "CameraLeft",	[KEY_CAMERA_RIGHT] =3D "CameraRight"=
,
+	[KEY_CAMERA_UP] =3D "CameraUp",		[KEY_CAMERA_ZOOMIN] =3D "CameraZoomIn",
+	[KEY_CAMERA_ZOOMOUT] =3D "CameraZoomOut",	[KEY_CLEARVU_SONAR] =3D "Clear=
VUSonar",
+	[KEY_CONTEXT_MENU] =3D "ContextMenu",	[KEY_DATA] =3D "Data",
+	[KEY_DATABASE] =3D "DataBase",		[KEY_DISPLAY_OFF] =3D "DisplayOff",
+	[KEY_DISPLAYTOGGLE] =3D "DisplayToggle",	[KEY_DOLLAR] =3D "Dollar",
+	[KEY_DUAL_RANGE_RADAR] =3D "DualRangeRadat",
+	[KEY_EDITOR] =3D "Editor",		[KEY_EURO] =3D "Euro",
+	[KEY_FASTREVERSE] =3D "FastReverse",	[KEY_FISHING_CHART] =3D "FishingCha=
rt",
+	[KEY_FN_RIGHT_SHIFT] =3D "FnRightShift",	[KEY_FRAMEBACK] =3D "FrameBack"=
,
+	[KEY_FRAMEFORWARD] =3D "FrameForward",	[KEY_FULL_SCREEN] =3D "FullScreen=
",
+	[KEY_GAMES] =3D "Games",			[KEY_GRAPHICSEDITOR] =3D "GraphicsEditor",
+	[KEY_HANGEUL] =3D "HanGeul",		[KEY_HANGUP_PHONE] =3D "HangUpPhone",
+	[KEY_IMAGES] =3D "Images",		[KEY_KBD_LCD_MENU1] =3D "KbdLcdMenu1",
+	[KEY_KBD_LCD_MENU2] =3D "KbdLcdMenu2",	[KEY_KBD_LCD_MENU3] =3D "KbdLcdMe=
nu3",
+	[KEY_KBD_LCD_MENU4] =3D "KbdLcdMenu4",	[KEY_KBD_LCD_MENU5] =3D "KbdLcdMe=
nu5",
+	[KEY_LEFT_DOWN] =3D "LeftDown",		[KEY_LEFT_UP] =3D "LeftUp",
+	[KEY_LIGHTS_TOGGLE] =3D "LightToggle",	[KEY_MACRO_PRESET1] =3D "MacroPre=
set1",
+	[KEY_MACRO_PRESET2] =3D "MacroPreset2",	[KEY_MACRO_PRESET3] =3D "MacroPr=
est3",
+	[KEY_MACRO_PRESET_CYCLE] =3D "MacroPresetCycle",
+	[KEY_MACRO_RECORD_START] =3D "MacroRecordStart",
+	[KEY_MACRO_RECORD_STOP] =3D "MacroRecordStop",
+	[KEY_MARK_WAYPOINT] =3D "MarkWayPoint",	[KEY_MEDIA_REPEAT] =3D "MediaRep=
eat",
+	[KEY_MEDIA_TOP_MENU] =3D "MediaTopMenu",	[KEY_MESSENGER] =3D "Messanger"=
,
+	[KEY_NAV_CHART] =3D "NavChar",		[KEY_NAV_INFO] =3D "NavInfo",
+	[KEY_NEWS] =3D "News",			[KEY_NEXT_ELEMENT] =3D "NextElement",
+	[KEY_NEXT_FAVORITE] =3D "NextFavorite",	[KEY_NOTIFICATION_CENTER] =3D "N=
otificationCenter",
+	[KEY_NUMERIC_0] =3D "Numeric0",		[KEY_NUMERIC_1] =3D "Numeric1",
+	[KEY_NUMERIC_11] =3D "Numceric11",	[KEY_NUMERIC_12] =3D "Numeric12",
+	[KEY_NUMERIC_2] =3D "Numeric2",		[KEY_NUMERIC_3] =3D "Numeric3",
+	[KEY_NUMERIC_4] =3D "Numeric4",		[KEY_NUMERIC_5] =3D "Numeric5",
+	[KEY_NUMERIC_6] =3D "Numeric6",		[KEY_NUMERIC_7] =3D "Numeric7",
+	[KEY_NUMERIC_8] =3D "Numeric8",		[KEY_NUMERIC_9] =3D "Numeric9",
+	[KEY_NUMERIC_A] =3D "NumericA",		[KEY_NUMERIC_B] =3D "NumericB",
+	[KEY_NUMERIC_C] =3D "NumericC",		[KEY_NUMERIC_D] =3D "NumericD",
+	[KEY_NUMERIC_POUND] =3D "NumericPound",	[KEY_NUMERIC_STAR] =3D "NumericS=
tar",
+	[KEY_ONSCREEN_KEYBOARD] =3D "OnScreenKeyBoard",
+	[KEY_PAUSE_RECORD] =3D "PauseRecord",	[KEY_PICKUP_PHONE] =3D "PickUpPhon=
e",
+	[KEY_PRESENTATION] =3D "Presentation",	[KEY_PREVIOUS_ELEMENT] =3D "Previ=
ousElement",
+	[KEY_PRIVACY_SCREEN_TOGGLE] =3D "PrivacyScreenToggle",
+	[KEY_RADAR_OVERLAY] =3D "RadarOverLay",
+	[KEY_RFKILL] =3D "RFKill",		[KEY_RIGHT_DOWN] =3D "RightDown",
+	[KEY_RIGHT_UP] =3D "RightUp",		[KEY_ROOT_MENU] =3D "RootMenu",
+	[KEY_ROTATE_LOCK_TOGGLE] =3D "RotateLockToggle",
+	[KEY_SCALE] =3D "Scale",			[KEY_SELECTIVE_SCREENSHOT] =3D "SelectiveScre=
enshot",
+	[KEY_SIDEVU_SONAR] =3D "SideVUSonar",	[KEY_SINGLE_RANGE_RADAR] =3D "Sing=
leRangeRadar",
+	[KEY_SLOWREVERSE] =3D "SlowReverse",	[KEY_SOS] =3D "SOS",
+	[KEY_SPREADSHEET] =3D "SpreadSheet",	[KEY_STOP_RECORD] =3D "StopRecord",
+	[KEY_TOUCHPAD_OFF] =3D "TouchPadOff",	[KEY_TOUCHPAD_ON] =3D "TouchPadOn"=
,
+	[KEY_TOUCHPAD_TOGGLE] =3D "TouchPadToggle",
+	[KEY_TRADITIONAL_SONAR] =3D "TraditionalSonar",
+	[KEY_UNMUTE] =3D "Unmute",		[KEY_UWB] =3D "UWB",
+	[KEY_VIDEO_NEXT] =3D "VideoNext",		[KEY_VIDEOPHONE] =3D "VideoPhone",
+	[KEY_VIDEO_PREV] =3D "VideoPrev",		[KEY_VOD] =3D "VOD",
+	[KEY_VOICEMAIL] =3D "VoiceMail",		[KEY_WLAN] =3D "WLAN",
+	[KEY_WORDPROCESSOR] =3D "WordProcessor",	[KEY_WPS_BUTTON] =3D "WPSButton=
",
+	[KEY_WWAN] =3D "WWAN",			[KEY_ZOOMIN] =3D "ZoomIn",
+	[KEY_ZOOMOUT] =3D "ZoomOut",		[KEY_ZOOMRESET] =3D "ZoomReset",
 };

 static const char *relatives[REL_MAX + 1] =3D {
@@ -1003,6 +3492,8 @@ static const char *relatives[REL_MAX + 1] =3D {
 	[REL_RY] =3D "Ry",		[REL_RZ] =3D "Rz",
 	[REL_HWHEEL] =3D "HWheel",	[REL_DIAL] =3D "Dial",
 	[REL_WHEEL] =3D "Wheel",		[REL_MISC] =3D "Misc",
+	[REL_WHEEL_HI_RES] =3D "WheelHiRes",
+	[REL_HWHEEL_HI_RES] =3D "HWheelHiRes"
 };

 static const char *absolutes[ABS_CNT] =3D {
@@ -1020,6 +3511,7 @@ static const char *absolutes[ABS_CNT] =3D {
 	[ABS_TILT_Y] =3D "YTilt",		[ABS_TOOL_WIDTH] =3D "ToolWidth",
 	[ABS_VOLUME] =3D "Volume",	[ABS_PROFILE] =3D "Profile",
 	[ABS_MISC] =3D "Misc",
+	[ABS_MT_SLOT] =3D "MTSlot",
 	[ABS_MT_TOUCH_MAJOR] =3D "MTMajor",
 	[ABS_MT_TOUCH_MINOR] =3D "MTMinor",
 	[ABS_MT_WIDTH_MAJOR] =3D "MTMajorW",
@@ -1029,11 +3521,17 @@ static const char *absolutes[ABS_CNT] =3D {
 	[ABS_MT_POSITION_Y] =3D "MTPositionY",
 	[ABS_MT_TOOL_TYPE] =3D "MTToolType",
 	[ABS_MT_BLOB_ID] =3D "MTBlobID",
+	[ABS_MT_TRACKING_ID] =3D "MTTrackingID",
+	[ABS_MT_PRESSURE] =3D "MTPressure",
+	[ABS_MT_DISTANCE] =3D "MTDistance",
+	[ABS_MT_TOOL_X] =3D "MTToolX",
+	[ABS_MT_TOOL_Y] =3D "MTToolY",
 };

 static const char *misc[MSC_MAX + 1] =3D {
 	[MSC_SERIAL] =3D "Serial",	[MSC_PULSELED] =3D "Pulseled",
-	[MSC_GESTURE] =3D "Gesture",	[MSC_RAW] =3D "RawData"
+	[MSC_GESTURE] =3D "Gesture",	[MSC_RAW] =3D "RawData",
+	[MSC_SCAN] =3D "Scan",		[MSC_TIMESTAMP] =3D "TimeStamp",
 };

 static const char *leds[LED_MAX + 1] =3D {
@@ -1041,7 +3539,8 @@ static const char *leds[LED_MAX + 1] =3D {
...
@@ -1053,11 +3552,32 @@ static const char *sounds[SND_MAX + 1] =3D {
 	[SND_TONE] =3D "Tone"
 };

+static const char *software[SW_CNT] =3D {
+	[SW_LID] =3D "Lid",
+	[SW_TABLET_MODE] =3D "TabletMode",
+	[SW_HEADPHONE_INSERT] =3D "HeadPhoneInsert",
+	[SW_RFKILL_ALL] =3D "RFKillAll",
+	[SW_MICROPHONE_INSERT] =3D "MicrophoneInsert",
+	[SW_DOCK] =3D "Dock",
+	[SW_LINEOUT_INSERT] =3D "LineOutInsert",
+	[SW_JACK_PHYSICAL_INSERT] =3D "JackPhysicalInsert",
+	[SW_VIDEOOUT_INSERT] =3D "VideoOutInsert",
+	[SW_CAMERA_LENS_COVER] =3D "CameraLensCover",
+	[SW_KEYPAD_SLIDE] =3D "KeyPadSlide",
+	[SW_FRONT_PROXIMITY] =3D "FrontProximity",
+	[SW_ROTATE_LOCK] =3D "RotateLock",
+	[SW_LINEIN_INSERT] =3D "LineInInsert",
+	[SW_MUTE_DEVICE] =3D "MuteDevice",
+	[SW_PEN_INSERTED] =3D "PenInserted",
