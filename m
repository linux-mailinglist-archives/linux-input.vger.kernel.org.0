...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id B2C891380F5
	for <lists+linux-input@lfdr.de>; Sat, 11 Jan 2020 11:43:40 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1728907AbgAKKnj (ORCPT <rfc822;lists+linux-input@lfdr.de>);
        Sat, 11 Jan 2020 05:43:39 -0500
Received: from mail.kernel.org ([198.145.29.99]:37280 "EHLO mail.kernel.org"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S1728861AbgAKKni (ORCPT <rfc822;linux-input@vger.kernel.org>);
        Sat, 11 Jan 2020 05:43:38 -0500
Received: from archlinux (cpc149474-cmbg20-2-0-cust94.5-4.cable.virginm.net [82.4.196.95])
        (using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
        (No client certificate requested)
        by mail.kernel.org (Postfix) with ESMTPSA id 682D820848;
        Sat, 11 Jan 2020 10:43:34 +0000 (UTC)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=kernel.org;
        s=default; t=1578739416;
        bh=nL3Xcp5LSkCpEL+XxmbXE/ZTezSjAsWy27clUnEFCss=;
        h=Date:From:To:Cc:Subject:In-Reply-To:References:From;
        b=QFeVt7poMTJXRYPmMKyEzrL2umTMrlPuEY4eGfLiOOJBffhw8BUYiyRIENSSe5Mbh
         VhTfROmvPc7khlW5AQ5XspegmBvVvgbLUS3FwFx0dS3zeyXtgAKI9A/yVY4oxXs0OX
         IGgXhBd+faz+LSjWTFGrUdkvAkOwQQbN8idvAAkI=
Date:   Sat, 11 Jan 2020 10:43:31 +0000
...
Subject: Re: [PATCH 2/2 v3] iio: light: Add a driver for Sharp GP2AP002x00F
Message-ID: <20200111104331.2268e837@archlinux>
In-Reply-To: <20200110100127.15519-2-linus.walleij@linaro.org>
References: <20200110100127.15519-1-linus.walleij@linaro.org>
        <20200110100127.15519-2-linus.walleij@linaro.org>
...
On Fri, 10 Jan 2020 11:01:27 +0100
...
A few minor bits remaining. See inline.  In particular you need
to get rid of all the of_ specific parts, rather than almost
all of them.
...
>  drivers/iio/light/gp2ap002.c | 723 +++++++++++++++++++++++++++++++++++
...
> index 000000000000..7ecdd296dfcd
> --- /dev/null
...
dev_dbg or we are going to spam the log a lot.
...
> +}
> +
> +static struct regmap_bus gp2ap002a00f_regmap_bus =3D {
> +	.reg_read =3D gp2ap002a00f_regmap_i2c_read,
> +	.reg_write =3D gp2ap002_regmap_i2c_write,
> +};
> +
> +static struct regmap_bus gp2ap002s00f_regmap_bus =3D {
> +	.reg_read =3D gp2ap002s00f_regmap_i2c_read,
> +	.reg_write =3D gp2ap002_regmap_i2c_write,
> +};
> +
...
> +		rbus =3D &gp2ap002s00f_regmap_bus;
> +	else
> +		rbus =3D &gp2ap002a00f_regmap_bus;
> +
> +	regmap =3D devm_regmap_init(dev, rbus, dev, &config);
> +	if (IS_ERR(regmap)) {
...
Is that really the only reason?  What if property was just not an integer?
Even if this is the only reason 'today' there is nothing to say that's
true long term.

Also, having gone to the effort of doing the device_property_read for
compatible, we should really be doing this for these as well.

so device_property_read_u8 etc.
...
Can we use of_property_read_u8 and avoid the need for a separate check?
...
Hmm. Seems like there is little point in continuing with this function if we
haven't managed to power up the regulator...
