Return-Path: <linux-input+bounces-16535-lists+linux-input=lfdr.de@vger.kernel.org>
X-Original-To: lists+linux-input@lfdr.de
Delivered-To: lists+linux-input@lfdr.de
Received: from sea.lore.kernel.org (sea.lore.kernel.org [IPv6:2600:3c0a:e001:db::12fc:5321])
	by mail.lfdr.de (Postfix) with ESMTPS id A5DD0CB49BD
	for <lists+linux-input@lfdr.de>; Thu, 11 Dec 2025 04:09:21 +0100 (CET)
Received: from smtp.subspace.kernel.org (conduit.subspace.kernel.org [100.90.174.1])
	by sea.lore.kernel.org (Postfix) with ESMTP id E14E2300FFB2
	for <lists+linux-input@lfdr.de>; Thu, 11 Dec 2025 03:08:46 +0000 (UTC)
Received: from localhost.localdomain (localhost.localdomain [127.0.0.1])
	by smtp.subspace.kernel.org (Postfix) with ESMTP id 3332B23645D;
	Thu, 11 Dec 2025 03:08:46 +0000 (UTC)
Authentication-Results: smtp.subspace.kernel.org;
	dkim=pass (2048-bit key) header.d=endrift.com header.i=@endrift.com header.b="kOtJA64R"
X-Original-To: linux-input@vger.kernel.org
Received: from endrift.com (endrift.com [173.255.198.10])
	(using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
	(No client certificate requested)
	by smtp.subspace.kernel.org (Postfix) with ESMTPS id 3B49121A457
	for <linux-input@vger.kernel.org>; Thu, 11 Dec 2025 03:08:42 +0000 (UTC)
Authentication-Results: smtp.subspace.kernel.org; arc=none smtp.client-ip=173.255.198.10
ARC-Seal:i=1; a=rsa-sha256; d=subspace.kernel.org; s=arc-20240116;
	t=1765422526; cv=none; b=pbiW5dapW2iJXzq3izgKKU4BW6iNy6auvlzIazwPtAC8MdRmjPT3z6wImNmqnmE/rmFJPY/YZgl9/GTv747RXYt9EuftR5B6H4IBffMkcjomNHpSJsQi44bzC+9l2dLglSDa+uV2zIXFCnTzQ8hXpCAjuAQePCF3/Gih6OaKzso=
ARC-Message-Signature:i=1; a=rsa-sha256; d=subspace.kernel.org;
	s=arc-20240116; t=1765422526; c=relaxed/simple;
	bh=GwROouDXH2OdUDZ5QNXbo0oaBpeXFMWWd8Y0mdtFtZY=;
	h=Message-ID:Date:MIME-Version:Subject:To:Cc:References:From:
	 In-Reply-To:Content-Type; b=C4vZru8vr/Gx2uHjQ3UCcNb85/Wr+jRDZ1yU1yBlUzsVopF9xNMV/nEI0oJ170cQHKjbICib99zIX1X0Vm2O8B23/40K8/MezXryQXI0k4wJKq7BhKBhuwwl3+dT8Usex6sXGAu0VSaETsgJkRSRyxHyiZVlWTNghR0E+qT/xKY=
ARC-Authentication-Results:i=1; smtp.subspace.kernel.org; dmarc=pass (p=none dis=none) header.from=endrift.com; spf=pass smtp.mailfrom=endrift.com; dkim=pass (2048-bit key) header.d=endrift.com header.i=@endrift.com header.b=kOtJA64R; arc=none smtp.client-ip=173.255.198.10
Authentication-Results: smtp.subspace.kernel.org; dmarc=pass (p=none dis=none) header.from=endrift.com
Authentication-Results: smtp.subspace.kernel.org; spf=pass smtp.mailfrom=endrift.com
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=endrift.com; s=2020;
	t=1765422519; bh=GwROouDXH2OdUDZ5QNXbo0oaBpeXFMWWd8Y0mdtFtZY=;
	h=Date:Subject:To:Cc:References:From:In-Reply-To:From;
	b=
Received: from [192.168.0.22] (71-212-14-89.tukw.qwest.net [71.212.14.89])
	by endrift.com (Postfix) with ESMTPSA id 032B8A018;
	Wed, 10 Dec 2025 19:08:38 -0800 (PST)
Message-ID: <b54fa61f-c905-44d9-8aca-c4b41f04a501@endrift.com>
Date: Wed, 10 Dec 2025 19:08:38 -0800
Precedence: bulk
...
To: Lyude Paul <lyude@redhat.com>, Dmitry Torokhov
 <dmitry.torokhov@gmail.com>, Jiri Kosina <jikos@kernel.org>,
 Benjamin Tissoires <bentiss@kernel.org>, linux-input@vger.kernel.org
Cc: Luiz Augusto von Dentz <luiz.dentz@gmail.com>,
 Bastien Nocera <hadess@hadess.net>
References: <20251120022457.469342-1-vi@endrift.com>
 <b391d34aeeb5140af39cf8b2b6dead858f9ec149.camel@redhat.com>
...
> Hi there! For those following along - I've worked a bit in input (and a=
cross
> the kernel for at least a decade) and figured I'd help a bit with revie=
w at
> Vicki's request
>=20
> On Wed, 2025-11-19 at 18:24 -0800, Vicki Pfau wrote:
>> This adds a new driver for the Switch 2 controllers. The Switch 2 uses=
 an
>> unusual split-interface design such that input and rumble occur on the=
 main
>> HID interface, but all other communication occurs over a "configuratio=
n"
>> interface. This is the case on both USB and Bluetooth, so this new dri=
ver
>> uses a split-driver design with the HID interface being the "main" dri=
ver
>> and the configuration interface is a secondary driver that looks up to=
 the
...
>> communications being extremely limited in the kernel, a custom interfa=
ce
>> between userspace and the kernel will need to be design, along with br=
ingup
...
>>
>> This initial work supports general input for the Joy-Con 2, Pro Contro=
ller
...
>=20
> I mentioned this on telegram but for the record: it might be worth cons=
idering
> if we need this as an actual build option or not

For the record, I'm only following the convention set by other drivers wi=
th force feedback. I'm A-OK dropping this flag if desired by subsystem ma=
intainers, and I have a weak preference for removing it as well. But I'd =
defer to the subsystem maintainers here.
...
>> + *   https://gist.github.com/shinyquagsire23/66f006b46c56216acbaac6c1=
e2279b64
...
>> +
>> +/*
>> + * The highest rumble level for "HD Rumble" is strong enough to poten=
tially damage the controller,
>> + * and also leaves your hands feeling like melted jelly, so we set a =
semi-abitrary scaling factor
>> + * to artificially limit the maximum for safety and comfort. It is cu=
rrently unknown if the Switch
>> + * 2 itself does something similar, but it's quite likely.
>> + *
>> + * This value must be between 0 and 1024, otherwise the math below wi=
ll overflow.
>> + */
>> +#define RUMBLE_MAX 450u
>> +
>> +/*
>> + * Semi-arbitrary values used to simulate the "rumble" sensation of a=
n eccentric rotating
>> + * mass type haptic motor on the Switch 2 controllers' linear resonan=
t actuator type haptics.
>> + *
>> + * The units used are unknown, but the values must be between 0 and 1=
023.
>> + */
...
>> +};
>> +
>> +static const struct switch2_ctlr_button_mapping left_joycon_button_ma=
ppings[] =3D {
>> +	{ BTN_TL,	0, NS2_BTNL_L,		},
...
>> +};
>> +
>> +static const struct switch2_ctlr_button_mapping right_joycon_button_m=
appings[] =3D {
...
>> +#endif
>> +
>> +static const uint8_t switch2_player_pattern[] =3D { 0x1, 0x3, 0x7, 0x=
f, 0x9, 0x5, 0xd, 0x6 };
>> +
>> +static const uint8_t switch2_feature_mask[] =3D {
>> +	NS2_FEATURE_BUTTONS | NS2_FEATURE_ANALOG | NS2_FEATURE_IMU | NS2_FEA=
TURE_RUMBLE,
>> +	0x00, 0x00, 0x00
>> +};
>> +
...
>> +
>> +#ifdef CONFIG_SWITCH2_FF
>> +static void switch2_encode_rumble(struct switch2_hd_rumble *rumble, u=
int8_t buffer[5])
>> +{
...
>> +}
>> +
>> +static int switch2_play_effect(struct input_dev *dev, void *data, str=
uct ff_effect *effect)
>> +{
...
>> +	} else {
>> +		ns2->rumble.hd.hi_amp =3D effect->u.rumble.weak_magnitude * RUMBLE_=
MAX >> 16;
>> +		ns2->rumble.hd.lo_amp =3D effect->u.rumble.strong_magnitude * RUMBL=
E_MAX >> 16;
>> +	}
>> +
...
>=20
> While it's technically impossible for this allocation to fail (since it=
's
> certainly < PAGE_SIZE) we probably should still error check here anyhow=
 in
...
>> +		ret =3D -ENODEV;
>=20
> I think this is backwards - usually with workqueues tied to a specific =
device
> in the kernel you don't want the workqueue in charge of checking whethe=
r the
> device is present or not. You just want the device to cancel the workqu=
eue
> unconditionally during driver shutdown, before doing any teardown that =
could
> put the device into a state where portions of its data aren't initializ=
ed.

I actually do cancel the work when tearing down the hdev. I'm not sure if=
 I added this for extra safety or if I added this due to debugging that I=
 didn't remove, but either way while this might be redundant, I guess it'=
s also theoretically safer in case there's a bug where it gets reschedule=
d somewhere. I'll take a closer look to see if I can figure out if it's e=
ven doing anything.
...
>=20
> BUT - you should also consider maybe using devm_mutex_init() (and the m=
any
> other fun devm function variants, which make resources tied to devices)=
 for
> this and other mutexes mutex instead.

I mentioned this on Telegram, but the primary motivation I have for not u=
sing devres more widely in these drivers is the discovery that if a drive=
r gets rmmod-ed without the device itself being removed, that memory "lea=
ks". While it's not a "real" leak, as it will be cleaned up if/when the d=
evice itself gets removed, it does mean that there's unused memory that c=
ould be freed more eagerly. I don't know if maintainers care about this s=
emi-leak in general, but for e.g. hid drivers where the device they're bo=
und to isn't necessarily going to be freed immediately following the hid =
driver shutdown, I prefer intentional cleanup. This seems like an oversig=
ht in how devres is designed and/or used, but since drivers being detache=
d and reattached repeatedly is a pretty rare scenario I'm not sure how mu=
ch it matters. USB is more common, however, as e.g. SDL now has code to d=
etach the USB device from the kernel driver if it wants to use it directl=
y. If that happens repeatedly, unused memory will pile up until the devic=
e is actually removed.

>=20
>> +}
>> +EXPORT_SYMBOL_GPL(switch2_controller_put);
>> +
>> +static bool switch2_parse_stick_calibration(struct switch2_stick_cali=
bration *calib,
>> +	const uint8_t *data)
>> +{
...
>> +}
>> +
>> +static void switch2_handle_flash_read(struct switch2_controller *ns2,=
 uint8_t size,
...
>> +			return;
>=20
> What's the intent with these conditionals? If it's just checking to mak=
e sure
> that *data is large enough to hold this, I would assume we probably wan=
t this
> to be:
>=20
...
> 	return;
>=20
> Since a mismatched size seems like it would be a pretty exceptional (e.=
g.
> something went very wrong, or a programmer mistake) error we'd want to =
know
> about. (same for the other conditionals in this function).
>=20
> (If this is actually something we expect to happen regularly, feel free=
 to
> ignore this comment)

We do not expect this to happen, generally speaking, but due to the afore=
mentioned USB device detaching from userspace, I don't really want to mak=
e any implicit assumptions about what we get being the same as what we'd =
request. If it's something that doesn't match what we expect, just reject=
 it and carry on.
...
>> +		} else {
>> +			hid_dbg(ns2->hdev, "Factory secondary stick calibration not presen=
t\n");
>> +		}
>> +		break;
...
>> +		return -EINVAL;
>> +
>> +	print_hex_dump_debug("got cmd: ", DUMP_PREFIX_OFFSET, 16, 1, message=
, length, false);
>> +
...
>> +
>> +static int switch2_features_enable(struct switch2_controller *ns2, in=
t features)
>> +{
...
>> +	if (!ns2->cfg)
>> +		return -ENOTCONN;
>> +	return ns2->cfg->send_command(NS2_CMD_FEATSEL, NS2_SUBCMD_FEATSEL_EN=
ABLE,
>> +		&feature_bits, sizeof(feature_bits),
>> +		ns2->cfg);
>> +}
>> +
>> +static int switch2_read_flash(struct switch2_controller *ns2, uint32_=
t address,
>> +	uint8_t size)
>> +{
...
>> +	return ns2->cfg->send_command(NS2_CMD_FLASH, NS2_SUBCMD_FLASH_READ, =
message,
>> +		sizeof(message), ns2->cfg);
>> +}
>> +
>> +static int switch2_set_player_id(struct switch2_controller *ns2, uint=
32_t player_id)
>> +{
...
>> +		input_set_abs_params(input, ABS_X, NS2_AXIS_MIN, NS2_AXIS_MAX, 32, =
128);
>> +		input_set_abs_params(input, ABS_Y, NS2_AXIS_MIN, NS2_AXIS_MAX, 32, =
128);
...
>> +		input_set_abs_params(input, ABS_RX, NS2_AXIS_MIN, NS2_AXIS_MAX, 32,=
 128);
>> +		input_set_abs_params(input, ABS_RY, NS2_AXIS_MIN, NS2_AXIS_MAX, 32,=
 128);
...
>=20
> Are we sure we need this TODO comment? it seems like you do check (or p=
ass
> that responsibility off to the caller of this function, which is fine)

This comment is again here just in case the driver gets detached by users=
pace and then reattached later. Within a normal, constrained flow it shou=
ldn't go wrong, but again I don't want to make any assumptions that nothi=
ng goes wrong. It's left as a TODO for now just as a reminder to shake ou=
t any assumptions later. As-is, if any singular step fails we'll be left =
with a weirdly partially-initialized device too, and we'd presumably want=
 to retry the step instead of just continuing on.
...
>> +			return ns2->cfg->send_command(NS2_CMD_FW_INFO, NS2_SUBCMD_FW_INFO_=
GET,
>> +				NULL, 0, ns2->cfg);
>> +			break;
>> +		case NS2_INIT_READ_FACTORY_PRIMARY_CALIB:
>> +			return switch2_read_flash(ns2, NS2_FLASH_ADDR_FACTORY_PRIMARY_CALI=
B,
>> +				NS2_FLASH_SIZE_FACTORY_AXIS_CALIB);
>> +		case NS2_INIT_READ_FACTORY_SECONDARY_CALIB:
>> +			if (switch2_ctlr_is_joycon(ns2->ctlr_type))
>> +				break;
>> +			return switch2_read_flash(ns2, NS2_FLASH_ADDR_FACTORY_SECONDARY_CA=
LIB,
>> +				NS2_FLASH_SIZE_FACTORY_AXIS_CALIB);
>> +		case NS2_INIT_READ_FACTORY_TRIGGER_CALIB:
>> +			if (ns2->ctlr_type !=3D NS2_CTLR_TYPE_GC)
>> +				break;
>> +			return switch2_read_flash(ns2, NS2_FLASH_ADDR_FACTORY_TRIGGER_CALI=
B,
...
>> +			return ns2->cfg->send_command(NS2_CMD_FEATSEL, NS2_SUBCMD_FEATSEL_=
SET_MASK,
...
>> +			if (!switch2_ctlr_is_joycon(ns2->ctlr_type))
>> +				break;
>> +			return ns2->cfg->send_command(NS2_CMD_GRIP, NS2_SUBCMD_GRIP_ENABLE=
_BUTTONS,
...
>> +
>> +static void switch2_report_buttons(struct input_dev *input, const uin=
t8_t *bytes,
>> +	const struct switch2_ctlr_button_mapping button_mappings[])
>> +{
>> +	const struct switch2_ctlr_button_mapping *button;
>> +
>> +	for (button =3D button_mappings; button->code; button++)
>> +		input_report_key(input, button->code, bytes[button->byte] & button-=
>bit);
>> +}
>> +
>> +static void switch2_report_axis(struct input_dev *input, struct switc=
h2_axis_calibration *calib,
...
>> +}
>> +
>> +static void switch2_report_stick(struct input_dev *input, struct swit=
ch2_stick_calibration *calib,
>> +	int x, int y, const uint8_t *data)
>> +{
>> +	switch2_report_axis(input, &calib->x, x, data[0] | ((data[1] & 0x0F)=
 << 8), false);
>> +	switch2_report_axis(input, &calib->y, y, (data[1] >> 4) | (data[2] <=
< 4), true);
>> +}
>> +
>> +static void switch2_report_trigger(struct input_dev *input, uint8_t z=
ero, int abs, uint8_t data)
>> +{
...
>> +}
>> +
>> +static int switch2_event(struct hid_device *hdev, struct hid_report *=
report, uint8_t *raw_data,
>> +	int size)
>> +{
...
>> +		/*
>> +		 * TODO
>> +		 * This won't be sent unless the report type gets changed via comma=
nd
...
>> +		switch2_report_stick(input, &ns2->stick_calib[0], ABS_X, ABS_Y, &ra=
w_data[6]);
>> +		switch2_report_buttons(input, &raw_data[3], left_joycon_button_mapp=
ings);
>> +		break;
>> +	case NS2_REPORT_JCR:
>> +		switch2_report_stick(input, &ns2->stick_calib[0], ABS_RX, ABS_RY, &=
raw_data[6]);
>> +		switch2_report_buttons(input, &raw_data[3], right_joycon_button_map=
pings);
>> +		break;
>> +	case NS2_REPORT_GC:
>> +		input_report_abs(input, ABS_HAT0X,
...
>> +		switch2_report_stick(input, &ns2->stick_calib[0], ABS_X, ABS_Y, &ra=
w_data[6]);
>> +		switch2_report_stick(input, &ns2->stick_calib[1], ABS_RX, ABS_RY, &=
raw_data[9]);
...
>> +}
>> +
>> +static int switch2_probe(struct hid_device *hdev, const struct hid_de=
vice_id *id)
>> +{
>> +	struct switch2_controller *ns2;
...
>> +	{ HID_USB_DEVICE(USB_VENDOR_ID_NINTENDO, USB_DEVICE_ID_NINTENDO_NS2_=
JOYCONL) },
>> +	{ HID_USB_DEVICE(USB_VENDOR_ID_NINTENDO, USB_DEVICE_ID_NINTENDO_NS2_=
JOYCONR) },
>> +	{ HID_USB_DEVICE(USB_VENDOR_ID_NINTENDO, USB_DEVICE_ID_NINTENDO_NS2_=
...
>> +};
>=20
> I may or may not be wrong here, but I think this (and any other structu=
res
> you're passing as-is to or from the device) should be __packed to make =
sure
...
>> +{
>> +	return type =3D=3D NS2_CTLR_TYPE_JCL || type =3D=3D NS2_CTLR_TYPE_JC=
R;
>> +}
>> diff --git a/drivers/input/joystick/Kconfig b/drivers/input/joystick/K=
config
...
>> +
>>  endif
>> diff --git a/drivers/input/joystick/Makefile b/drivers/input/joystick/=
Makefile
...
>> diff --git a/drivers/input/joystick/switch2-usb.c b/drivers/input/joys=
tick/switch2-usb.c
>> new file mode 100644
...
>> +}
>> +
>> +static int switch2_usb_send_cmd(enum switch2_cmd command, uint8_t sub=
command,
...
>> +		command, NS2_DIR_OUT | NS2_FLAG_OK, NS2_TRANS_USB, subcommand, 0, s=
ize
>> +	};
>> +
>> +	if (size > 56) {
...
>> +
>> +	print_hex_dump_debug("sending cmd: ", DUMP_PREFIX_OFFSET, 16, 1, urb=
->data,
...
>> +{
>> +	struct switch2_usb *ns2_usb =3D container_of(work, struct switch2_us=
b, message_in_work);
...
>> +	spin_unlock_irqrestore(&ns2_usb->bulk_in_lock, flags);
>> +
>> +	err =3D switch2_receive_command(ns2_usb->cfg.parent, urb->urb->trans=
fer_buffer,
...
>> +}
>> +
>> +static int switch2_usb_probe(struct usb_interface *intf, const struct=
 usb_device_id *id)
>> +{
>> +	struct switch2_controller *ns2;
...
>> +		return -EINVAL;
>> +
>> +	ret =3D usb_find_common_endpoints(intf->cur_altsetting, &bulk_in, &b=
ulk_out, NULL, NULL);
>> +	if (ret) {
...
>> +		}
>> +
>> +		ns2_usb->bulk_in[i].data =3D usb_alloc_coherent(udev, NS2_BULK_SIZE=
, GFP_KERNEL,
...
;
>> +		ns2_usb->bulk_in[i].urb->transfer_flags |=3D URB_NO_TRANSFER_DMA_MA=
P;
>> +	}
>> +
>> +	for (i =3D 0; i < NS2_OUT_URBS; i++) {
...
>> +		}
>> +
>> +		ns2_usb->bulk_out[i].data =3D usb_alloc_coherent(udev, NS2_BULK_SIZ=
E, GFP_KERNEL,
...
>> +			ns2_usb->bulk_out[i].data, NS2_BULK_SIZE, switch2_bulk_out, ns2_us=
b);
>> +		ns2_usb->bulk_out[i].urb->transfer_flags |=3D URB_NO_TRANSFER_DMA_M=
AP;
>> +	}
>> +
...
>> +err_free_out:
>> +	for (i =3D 0; i < NS2_OUT_URBS; i++) {
>> +		usb_free_coherent(ns2_usb->udev, NS2_BULK_SIZE, ns2_usb->bulk_out[i=
].data,
>> +			ns2_usb->bulk_out[i].urb->transfer_dma);
...
>> +err_free_in:
>> +	for (i =3D 0; i < NS2_IN_URBS; i++) {
>> +		usb_free_coherent(ns2_usb->udev, NS2_BULK_SIZE, ns2_usb->bulk_in[i]=
=2Edata,
>> +			ns2_usb->bulk_in[i].urb->transfer_dma);
...
>> +	USB_DEVICE_AND_INTERFACE_INFO(vend, prod, USB_CLASS_VENDOR_SPEC, 0, =
0)
>> +
>> +static const struct usb_device_id switch2_usb_devices[] =3D {
>> +	{ SWITCH2_CONTROLLER(USB_VENDOR_ID_NINTENDO, USB_DEVICE_ID_NINTENDO_=
NS2_JOYCONL) },
>> +	{ SWITCH2_CONTROLLER(USB_VENDOR_ID_NINTENDO, USB_DEVICE_ID_NINTENDO_=
NS2_JOYCONR) },
>> +	{ SWITCH2_CONTROLLER(USB_VENDOR_ID_NINTENDO, USB_DEVICE_ID_NINTENDO_=
