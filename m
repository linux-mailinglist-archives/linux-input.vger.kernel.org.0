...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id BE8ED23B8C5
	for <lists+linux-input@lfdr.de>; Tue,  4 Aug 2020 12:30:36 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1726450AbgHDKag (ORCPT <rfc822;lists+linux-input@lfdr.de>);
        Tue, 4 Aug 2020 06:30:36 -0400
Received: from out2-smtp.messagingengine.com ([66.111.4.26]:43375 "EHLO
        out2-smtp.messagingengine.com" rhost-flags-OK-OK-OK-OK)
        by vger.kernel.org with ESMTP id S1726056AbgHDKaf (ORCPT
        <rfc822;linux-input@vger.kernel.org>);
        Tue, 4 Aug 2020 06:30:35 -0400
Received: from compute7.internal (compute7.nyi.internal [10.202.2.47])
        by mailout.nyi.internal (Postfix) with ESMTP id B82565C0200;
        Tue,  4 Aug 2020 06:30:33 -0400 (EDT)
Received: from mailfrontend1 ([10.202.2.162])
  by compute7.internal (MEProxy); Tue, 04 Aug 2020 06:30:33 -0400
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed; d=
        messagingengine.com; h=cc:content-transfer-encoding:content-type
        :date:from:message-id:mime-version:subject:to:x-me-proxy
        :x-me-proxy:x-me-sender:x-me-sender:x-sasl-enc; s=fm3; bh=hwX2yc
        cmXXxntb8LnqR7KVqQqBs+7DX8xlt6GL8iLmM=; b=ZbFyoDulMyUtmS/tgcQ5X8
        
        ==
X-ME-Sender: <xms:yTgpX6dm0v2BKlT5uMpQOs9qITZotMnm8qK4GfSubeFun1ndb0tV7w>
X-ME-Proxy-Cause: gggruggvucftvghtrhhoucdtuddrgeduiedrjeeigdeftdcutefuodetggdotefrodftvf
    ...
    
    hnvghsrdguvghv
X-ME-Proxy: <xmx:yTgpX0Pbudast2E1QSn9HoqCBVG1JAQO9KnBgf3RnsmP0x3ZB8YDAg>
    <xmx:yTgpX7jlRJsU_aF4FvkkSrknIF8m5ZfTrBG_NO3dMKclhSxAkt3c7w>
    <xmx:yTgpX3-LPJ0rddRZ3FMXJJKwLlaP3YsQqn8MSNBAXOYu1toWah-_mQ>
    <xmx:yTgpX05fVIFJq4wRJnqnzHDNhzcvzfMi1zsswwt8BqtW_XXXRwJXPQ>
Received: from [192.168.20.21] (203-114-141-16.sta.inspire.net.nz [203.114.141.16])
        by mail.messagingengine.com (Postfix) with ESMTPA id 1C1DE3280059;
        Tue,  4 Aug 2020 06:30:31 -0400 (EDT)
Message-ID: <3e9ce2ae2e42c5c7e39070fc9bfcee89ebd5277a.camel@ljones.dev>
Subject: [PATCH V4]: HID: asus: add support for ASUS N-Key keyboard
From:   Luke Jones <luke@ljones.dev>
To:     linux-input@vger.kernel.org
Cc:     jikos@kernel.org, benjamin.tissoires@redhat.com
Date:   Tue, 04 Aug 2020 22:30:25 +1200
...
+	if ((usage->hid & HID_USAGE_PAGE) == HID_UP_ASUSVENDOR &&
 		...
 		hid_warn(hdev, "Unmapped Asus vendor usagepage code
0x%02x\n",
...
+			return -1;
+		/* Fn+F5 "fan" symbol, trigger WMI event to toggle next
mode */
...
+	if (ret < 0)
+		hid_err(hdev, "Asus failed to send init start command:
%d\n", ret);
+	/* Followed by a string */
+	ret = asus_kbd_set_report(hdev, buf_init2, sizeof(buf_init2));
+	if (ret < 0)
+		hid_err(hdev, "Asus failed to send init command 1.0:
%d\n", ret);
+	/* Followed by a string */
+	ret = asus_kbd_set_report(hdev, buf_init3, sizeof(buf_init3));
+	if (ret < 0)
+		hid_err(hdev, "Asus failed to send init command 1.1:
%d\n", ret);
+
...
+	ret = asus_kbd_set_report(hdev, buf_init2, sizeof(buf_init2));
+	if (ret < 0)
+		hid_err(hdev, "Asus failed to send init command 2.0:
%d\n", ret);
+
+	ret = asus_kbd_set_report(hdev, buf_init3, sizeof(buf_init3));
+	if (ret < 0)
+		hid_err(hdev, "Asus failed to send init command 2.1:
%d\n", ret);
+
+	return ret;
+}
...
+
+	if (drvdata->quirks & QUIRK_ROG_NKEY_KEYBOARD) {
+		/* Initialize keyboard LED interface and Vendor keys on
0x1866 */
...
break;
	...
	break;
 		case 0x35: asus_map_key_clear(KEY_DISPLAY_OFF);		
break;
 		case 0x6c: asus_map_key_clear(KEY_SLEEP);		
break;
-		case 0x7c: asus_map_key_clear(KEY_MICMUTE);		
break;
+		case 0x7c: asus_map_key_clear(KEY_F20);		break;
 		case 0x82: asus_map_key_clear(KEY_CAMERA);		
break;
 		case 0x88: asus_map_key_clear(KEY_RFKILL);		
	break;
 		case 0xb5: asus_map_key_clear(KEY_CALC);		
	break;
@@ -771,16 +862,42 @@ static int asus_input_mapping(struct hid_device
*hdev,
 		/* ROG key */
 		case 0x38: asus_map_key_clear(KEY_PROG1);		
break;
 
-		/* Fn+C ASUS Splendid */
-		case 0xba: asus_map_key_clear(KEY_PROG2);		
break;
+		default:
+			if (drvdata->quirks & QUIRK_G752_KEYBOARD) {
+				switch (usage->hid & HID_USAGE) {
+				/* Fn+C ASUS Splendid */
+				case 0xba:
asus_map_key_clear(KEY_PROG2);		break;
...
+				/* Fn+Space Power4Gear Hybrid */
+				case 0x5c:
asus_map_key_clear(KEY_PROG3);		break;
 
-		/* Fn+F5 "fan" symbol on FX503VD */
-		case 0x99: asus_map_key_clear(KEY_PROG4);		
break;
+				/* Fn+F5 "fan" symbol on FX503VD */
+				case 0x99:
asus_map_key_clear(KEY_PROG4);		break;
+
+				default:
+					return -1;
+				}
...
{
+				switch (usage->hid & HID_USAGE) {
+				/* Fn+Ret "Calc" symbol on device
0x1866, N-KEY Device */
+				case 0x92:
asus_map_key_clear(KEY_CALC);		break;
+
+				/* Fn+Left Aura mode previous */
+				case 0xb2:
asus_map_key_clear(KEY_KBDILLUM_MODE_PREV);	break;
+
+				/* Fn+Right Aura mode next */
+				case 0xb3:
asus_map_key_clear(KEY_KBDILLUM_MODE_NEXT);	break;
+
+				default:
+					return -1;
+				}
...
-		default:
 			/* ASUS lazily declares 256 usages, ignore the
rest,
 			 * as some make the keyboard appear as a
pointer device. */
 			return -1;
...
+#define ASUS_WMI_METHODID_NOTIF		0x00100021 /* Notify
method ?? */
+
